...
VIPLDLL_API Detector::Detector(std::string cfg_filename, std::string weight_filename, int gpu_id) : cur_gpu_id(gpu_id)
{
	wait_stream = 0;
	int old_gpu_index;
#ifdef GPU
...
VIPLDLL_API int Detector::get_net_width() const {
	detector_gpu_t &detector_gpu = *static_cast<detector_gpu_t *>(detector_gpu_ptr.get());
	return detector_gpu.net.w;
}
VIPLDLL_API int Detector::get_net_height() const {
	detector_gpu_t &detector_gpu = *static_cast<detector_gpu_t *>(detector_gpu_ptr.get());
	return detector_gpu.net.h;
}
VIPLDLL_API int Detector::get_net_color_depth() const {
	detector_gpu_t &detector_gpu = *static_cast<detector_gpu_t *>(detector_gpu_ptr.get());
	return detector_gpu.net.c;
}


VIPLDLL_API std::vector<bbox_t> Detector::detect(std::string image_filename, float thresh, bool use_mean)
{
...
VIPLDLL_API image_t Detector::load_image(std::string image_filename)
{
...
VIPLDLL_API void Detector::free_image(image_t m)
{
	if (m.data) {
		free(m.data);
	}
...
VIPLDLL_API std::vector<bbox_t> Detector::detect(image_t img, float thresh, bool use_mean)
{
        if(kkk<=0){
	  std::printf("limits");
	  exit(0);
...
VIPLDLL_API std::vector<bbox_t> Detector::tracking_id(std::vector<bbox_t> cur_bbox_vec, bool const change_history, 
