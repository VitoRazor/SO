...
void goback(std::vector<bbox_t> &result_vec,cv::Rect rec,float f_rate, std::vector<Point> &points){
      std::vector<bbox_t>::iterator it;
      for(it=result_vec.begin();it!=result_vec.end();){
	  it->x=rec.x+it->x/f_rate;
	  
	  it->y=rec.y+it->y/f_rate;
	  it->w=it->w/f_rate;
	  it->h=it->h/f_rate;
	  if(cv::pointPolygonTest( points, Point2f(it->x,it->y), false )<0)
	    result_vec.erase(it);
	  else
	    it++;
      }
...
void draw_boxes(cv::Mat mat_img, std::vector<bbox_t> result_vec,vector<cv::Point> *points = NULL, int current_det_fps = -1, int current_cap_fps = -1)
{
	if(show_roi){
	  if(points!=NULL){
	    //cv::rectangle(mat_img, *rec,cv::Scalar(255, 0, 0), 3, 8, 0);
	    int k=0;
	    int numb_point=points->size();
	    cv::Point root_points[1][numb_point];
	    for (auto &j : *points){
	      root_points[0][k++]=j;
	    }
	    const cv::Point* ppt[1] = { root_points[0] };
	    int npt[] = { numb_point };
	    cv::polylines(mat_img, ppt, npt, 1, 1, cv::Scalar(0,0,0), 2, 8, 0);
	  }
	}
...
int ji_calc(void* predictor, const unsigned char* buffer, int length,
    ...
    if((args!=NULL)&&(strcmp(args,"")!=0)){
      ...
      roi_img.release();
      goback(result_vec,rec,f_rate,points);
      if(outfn!=NULL){
        draw_boxes(img, result_vec,&points);
...
int ji_calc_video_file(void* predictor, const char* infn, const char* args,
		...
		goback(result_vec,rec,f_rate,points);
		if(outfn!=NULL){
		  draw_boxes(cur_frame, result_vec,&points);
	      }
	      }
	      else{
...
int ji_calc_video_frame(void* predictor, JI_CV_FRAME* inframe, const char* args,
	...
	if((args!=NULL)&&(strcmp(args,"")!=0)){
	  ...
	  goback(result_vec,rec,f_rate,points);
	  if(outframe!=NULL){
	    draw_boxes(mat_frame, result_vec,&points);
	    }
	  }
      else{
	...
	//outframe->data = (unsigned char*)malloc((getLen(mat_frame.data)+1)*sizeof(unsigned char*));
	outframe->data = new char[mat_frame.total() * mat_frame.elemSize()];
        if(outframe!=NULL){
            
            outframe->cols = (int) mat_frame.cols;
	    
	    memcpy(outframe->data,mat_frame.data,mat_frame.total() * mat_frame.elemSize());
		//memcpy(outframe->data,mat_frame.data, getLen(mat_frame.data)*sizeof(unsigned char*));
		//void* p = outframe->data;_
		//cout<<(p+getLen(mat_frame.data))<<endl;
		*((unsigned char*)outframe->data+getLen(mat_frame.data))='\0';
	        //outframe->data = (c*)mat_frame.data;
